...
class ServiceController{
  ...
  static Future<void> createService({String area="", String category="", String description="", String name="", int? price, String unit="", String uid=""}) async{
    ...
    if (ViewVariables.file != null) {
      link = await Controller.saveImage(ViewVariables.file,
          "Services${DateTime.now().microsecondsSinceEpoch}");
    } else {
      print("nulled");
    ...
    //print(link);
    DocumentReference ref = await servicelist.add({
  ...
  static Future getService(String id) async{
    var mydoc = FirebaseFirestore.instance.collection("serviceinfo").doc(id);
    mydoc.get().then((mydoc) =>{
    item = mydoc
    });
    return item;
  }
